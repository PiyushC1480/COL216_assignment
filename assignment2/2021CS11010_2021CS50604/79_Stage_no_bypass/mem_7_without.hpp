...
int memory_1::load(int reg_addr, int PC, vector<vector<int> >& DM_DM){
    ...
    ifstream mem_file;
    string val; 
    reg_file.open("reg_7_without.txt");
    mem_file.open("data_mem_7_without.txt");
    int line_num = 0;
...
void memory_1::init(){
    ofstream reg_file;
    reg_file.open("data_mem_7_without.txt");
...
void memory_2::store(int reg_addr, int value){
    ...
    updated_mem.open("data_mem_7_without.txt");
...
void memory_2::write(int value, int offset,  int PC){
    ...
    mem_file.open("data_mem_7_without.txt",fstream::out| fstream::trunc);
