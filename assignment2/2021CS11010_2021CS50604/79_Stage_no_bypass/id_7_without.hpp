...
class instr_decode_2{
    public : 
        void OP_code(vector<vector<string> > & ID_ID, int PC, vector<vector<int> > & ID_RR);
        void upd_PC(int new_PC){PC = new_PC;}
        int opr_types(int opr);
        void data_hazards(vector<vector<int> >& ID_RR);
...
void instr_decode_2::OP_code(vector<vector<string> > & ID_ID, int PC, vector<vector<int> > & ID_RR){
    upd_PC(PC);
    bool end = false;
    ...
    if(not end) data_hazards(ID_RR);
}

void instr_decode_2::data_hazards(vector<vector<int> >& ID_RR){
    ...
    if (PC > 0){
        if (cycles_in_stall==0){
            ...
            if (prev_regs[0] == next_regs[1]){
                ...
                if (opcode == 18){
                    if (prev_regs[0] == next_regs[1] || prev_regs[0] == next_regs[1]){
                        if (cycles_in_stall==0){
                            cycles_in_stall = 2;
                        }
                    }
    ...
    if (PC > 1) {
        if (cycles_in_stall==0){
            ...
            if (prev_to_prev_regs[0] == next_regs[1] || prev_to_prev_regs[0] == next_regs[0]){
                if (cycles_in_stall==0){
                    cycles_in_stall = 1;
                }
            }
            ...
            if (prev_to_prev_regs[0] == next_regs[1]){
                ...
                if (opcode == 18){
                    if (prev_to_prev_regs[0] == next_regs[1] || prev_to_prev_regs[0] == next_regs[1]){
                        if (cycles_in_stall==0){
                            cycles_in_stall = 1;
                        }
                    }}
